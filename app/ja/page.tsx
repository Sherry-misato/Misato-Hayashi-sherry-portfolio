...
const sectionCards = [
  {
    ...
    title: "Introduction",
    ja: "自己紹介",
    body: "Who I am, what I study, and why health became the center of my interests.",
    href: "#about",
  },
  {
    label: "RESEARCH",
    title: "Medical AI",
    ja: "研究",
    body: "My interests in gut microbiome analysis, preventive healthcare, and applied informatics.",
    href: "#research",
  },
  {
    label: "ACTIVITIES",
    title: "Achievements",
    ja: "実績・活動",
    body: "Ballet, CanSat, SGX, interviews, and outreach experiences.",
    href: "#activities",
  },
  {
    label: "PERSONAL",
    title: "Life & Favorites",
    ja: "プライベート",
    body: "Dance, latte art, and the small challenges that show my curiosity in daily life.",
    href: "#personal",
  },
];
  ...
  return (
    <Box bg="#fffafb" color="ink">
      <Box as="header" position="sticky" top="0" zIndex="10" borderBottom="1px solid" borderColor="rose.100" bg="rgba(255,250,251,.9)" backdropFilter="blur(12px)">
        <Container maxW="6xl">
          <Flex h="68px" align="center" justify="space-between">
            <Link as={NextLink} href="/ja" _hover={{ textDecoration: "none" }}>
      ...
      <Box as="main">
        ...
        <Box bg="white" borderY="1px solid" borderColor="rose.100" py={{ base: 12, md: 16 }}>
          <Container maxW="6xl">
            <Text color="rose.700" fontSize="sm" letterSpacing=".12em">SITE GUIDE</Text>
            <Heading mt={3} fontSize={{ base: "2xl", md: "3xl" }} fontWeight="500">Four Ways to Know Me</Heading>
            <Text mt={2} color="gray.500">自己紹介・研究・活動・プライベート</Text>
            <SimpleGrid columns={{ base: 1, sm: 2, lg: 4 }} gap={4} mt={8}>
              {sectionCards.map((card) => (
                <Link key={card.label} href={card.href} _hover={{ textDecoration: "none" }}>
                  <Box h="100%" bg="#fffafb" border="1px solid" borderColor="rose.200" borderRadius="8px" p={6}>
                    <Text color="rose.700" fontSize="xs" letterSpacing=".14em" fontWeight="bold">{card.label}</Text>
                    <Heading as="h2" mt={4} fontSize="xl" fontWeight="500">{card.title}</Heading>
                    <Text mt={1} color="gray.500" fontSize="sm">{card.ja}</Text>
                    <Text mt={4} color="gray.600" fontSize="sm" lineHeight="1.8">{card.body}</Text>
                  </Box>
                </Link>
              ))}
            </SimpleGrid>
        ...
        <Box id="activities" bg="white" borderY="1px solid" borderColor="rose.100" py={{ base: 16, md: 24 }}>
          <Container maxW="6xl">
        ...
        <Container maxW="6xl" py={{ base: 16, md: 24 }}>
          <Grid templateColumns={{ base: "1fr", lg: "1fr 1fr" }} gap={{ base: 14, lg: 14 }}>
            ...
            <Box id="personal" scrollMarginTop="88px">
        ...
        <Box bg="#35453c" color="white" py={{ base: 16, md: 22 }}>
          <Container maxW="6xl">
            ...
            <Heading mt={4} maxW="850px" fontSize={{ base: "3xl", md: "5xl" }} lineHeight="1.45" fontWeight="500">
              Growing into a researcher who can work across borders.
            </Heading>
            <Text mt={4} color="rose.100" fontFamily="heading" fontSize={{ base: "xl", md: "2xl" }}>
              海外でも活躍できる力をつけ、医療AIを人の健康に届く価値へ。
            </Text>
            <Text mt={6} maxW="760px" color="whiteAlpha.850" lineHeight="2">
              My goal is to present research internationally, publish papers, and build the confidence and skill to contribute in global research environments.
            </Text>
            <Text mt={2} maxW="760px" color="whiteAlpha.700" lineHeight="1.9">
              国際会議・論文発表に挑み、海外でも活躍できる研究者を目指しています。
            </Text>
